...
export function normalizeComment(rawComment) {
  ...
  return {
    id: rawComment.id,
    body: rawComment.content ?? rawComment.body ?? '',
    author,
...
export const useFeedStore = defineStore('feed', {
  ...
  actions: {
    ...
    async toggleLike(post) {
      ...
      const newLikesCount = Math.max(0, post.likesCount + (newLikedByMe ? 1 : -1))

      this.applyPostPatch(post.id, { likedByMe: newLikedByMe, likesCount: newLikesCount })

      try {
        const action = post.likedByMe ? likesService.unlike : likesService.like
        return await action(post.id)
      } catch (error) {
        this.applyPostPatch(post.id, { likedByMe: post.likedByMe, likesCount: post.likesCount })
        throw error
      }
    },
